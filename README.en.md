...
The repository reserves [`assets/examples/`](assets/examples/) for future work. Only finished 034 artwork verified by the project owner will be added; no post or image from another style is used as a placeholder.

Future samples will demonstrate 034's adaptability. Their subjects, whitespace ratios, palette, copy, and canvas ratios will never become generation references or defaults.

## Four outputs, one rubber-stamp field-note system
...
| --- | --- | --- |
| `top-bottom` | source-adaptive | complete source above, 034 multicolour rubber-stamp field note below; both panels retain the source size and split exactly 50/50 |
| `left-right` | source-adaptive | complete source left, 034 multicolour rubber-stamp field note right; both panels retain the source size and split exactly 50/50 |
...
Exact user pixels > explicit ratio or destination > source adaptation for ordinary modes. The original `034.md` used a 3:4 creative canvas, but that historical example is not a silent default in the current Skill.
...
- **Independent set:** every device references only the source photograph and may explore different stamp scale, placement, crop, spot-ink separation, impression wear, whitespace, and annotation alignment.
...
Automatic copy derives one concise title from a verified place, subject, state, action, or supported image. It may add two or three grounded keywords or one tiny note, but never a tourism slogan or invented picturesque backstory.

`No.` appears only when supplied, reliably established, or transparently assigned as the current batch sequence; never invent historical metadata, coordinates, dates, or provenance. Use small native typewriter or archival microtype aligned with a stamp edge, baseline, local contour, or active whitespace. Copy must pass the unrelated-image swap test.
...
The image model creates the small stamp motif, carved line, two-to-four spot-ink separation, broken impression, slight misregistration, warm fibrous paper, generous whitespace, and native field annotation. `scripts/compose_panel.py` only plans canvases, performs exact 50/50 raster composition, finalises dimensions, and audits results. It never fakes artwork with programmatic drawing.
...
- [Chinese full prompt](references/xxd-panel-034-prompt.zh-CN.md)
- [English full prompt](references/xxd-panel-034-prompt.en.md)
